    ...
    var LessonDashboard = Widget.extend({
        ...
        _onChangeLessonId: function(id) {
            ...
            $.get(this.get('courseUri') + '/lesson/' + id, function(lesson) {
                ...
                if (lesson.mediaSource == 'Qiniu') {
                    console.log(lesson);
                    require('/video/assets/Qiniu/css/video-js.css');
                    require('/video/assets/Qiniu/js/video.min');
                    require('/video/assets/Qiniu/js/videojs-ie8.min');

                    var vType = function() {

                        var type = '';
                        $.ajax({
                            url: lesson.mediaUri + "?stat",
                            async: false
                        }).done(function(info) {
                            type = info.mimeType;
                    ...
                    $('#lesson-video-content').empty();
                    $('#lesson-video-content').append(player);

                    console.log('=======>>Type:', vType(), '====>>vLink:', lesson.mediaUri);
                    var poster = lesson.mediaUri + '?vframe/jpg/offset/2';
                    videojs('video-embed', {
                        "width": "100%",
                        "height": "100%",
                        "controls": true,
                ...
                } else if (lesson.mediaSource == 'iframe') {
